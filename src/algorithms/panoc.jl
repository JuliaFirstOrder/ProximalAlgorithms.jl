...
function Base.iterate(iter::PANOCIteration{R}) where R
    x = copy(iter.x0)
    Ax = iter.A * x
    f_Ax, pb = eval_with_pullback(iter.f, Ax)
    grad_f_Ax = pb()
...
function Base.iterate(iter::PANOCIteration{R, Tx, Tf}, state::PANOCState) where {R, Tx, Tf}
    ...
    state.f_Ax_d, pb = eval_with_pullback(iter.f, state.Ax_d)
    ...
    for k in 1:iter.max_backtracks
        ...
        if ProximalCore.is_generalized_quadratic(Tf)
            ...
            if isinf(f_Az)
                f_Az, pb = eval_with_pullback(iter.f, state.Az)
                state.grad_f_Az .= pb()
            end
            if isinf(c)
            ...
            state.f_Ax, pb = eval_with_pullback(iter.f, state.Ax)
            state.grad_f_Ax .= pb()
